...
{{ acl.model.short_name | replace('.', '_') }}_{{ group.name | replace('.', '_') }},True,"{{ acl.model.name}} {{ group.name }}",{{ acl.group_name() }},{{ module.name }}.model_{{ acl.model.name | replace('.', '_') }},{{ one_or_zero(acl.create) }},{{ one_or_zero(acl.read) }},{{ one_or_zero(acl.write) }},{{ one_or_zero(acl.delete) }}
{%- endfor -%}
{%- endfor -%}
